...
import { hasProductMobileBottomBar } from "../products/ProductMobileBottomBar";
...
export function CartPopup({
  ...
  const cartPopupBottom = hasProductMobileBottomBar(pathname) ? "8.25rem" : "5.25rem";

  // Detect open filter/sort panels, bottom sheets, and modals
  useEffect(() => {
    const checkPanels = () => {
      // Body overflow is set to 'hidden' by all open modals and bottom sheets
      const isBodyLocked = document.body.style.overflow === "hidden";

      // Also check for explicit open modal overlays
      const openOverlays = document.querySelectorAll(
        '.fixed.inset-0.opacity-100:not(.pointer-events-none), [aria-hidden="false"][role="dialog"]',
      );

      const hasVisibleOverlay = Array.from(openOverlays).some((el) => {
        const style = window.getComputedStyle(el);
        return style.display !== "none" && style.visibility !== "hidden" && style.opacity !== "0";
      });

      setIsPanelOpen(isBodyLocked || hasVisibleOverlay);
    };
   ...
   */
  useEffect(() => {
    const show = totalItems > 0 && !isExcludedPage && !isPanelOpen && !isSearchOpen;
  ...
  }, [totalItems, isExcludedPage, isPanelOpen, isSearchOpen]);
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed inset-x-0 z-[58] md:hidden
          ...
          >
            {recentItems.length > 0 && (
              <div className="flex items-center shrink-0">
                {avatarStack("w-11 h-11", "-ml-4")}
              </div>
            )}
            <div className="flex flex-col">
              <span className="text-[10px] font-medium uppercase tracking-wider text-[#D6A146]">
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col pr-2">
              <span className="text-[11px] font-medium uppercase tracking-wider text-[#D6A146]">
          ...
          >
            ...
            <div className="flex flex-col pr-2 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-medium text-[#D6A146] uppercase tracking-wider whitespace-nowrap">
                  {newItemAdded ? "Recently Added" : "Items in Cart"}
                </span>
              </div>
              <span className="text-sm font-medium text-white mt-0.5 whitespace-nowrap">
                {totalItems}{" "}
                {totalItems === 1 ? "Spice Pouch" : "Spice Pouches"}
              </span>

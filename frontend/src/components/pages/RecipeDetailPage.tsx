...
import { fetchRecipeBySlug, RecipeApiError } from "../../lib/recipeApi";
    ...
    return (
      <div className="min-h-screen bg-[#F7F5EF] pt-32 px-4">
        <div className="max-w-lg mx-auto rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
          ...
          <p className="text-sm text-red-700 mb-4">{error || "Recipe not found"}</p>
          <div className="flex items-center justify-center gap-3">
            <button
              onClick={onBack}
              className="text-xs font-semibold text-[#284C38] hover:underline"
            >
              Back to Recipes
            </button>
            <button
              onClick={() => void loadRecipe()}
              className="inline-flex items-center gap-1.5 text-xs font-bold text-[#284C38]"
            >

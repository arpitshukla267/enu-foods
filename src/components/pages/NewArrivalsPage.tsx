...
export const NewArrivalsPage: React.FC<NewArrivalsPageProps> = ({
  ...
  // New arrivals: items flagged with isNew or recent items reversed
  const newArrivalsList = useMemo(() => {
    const flaggedNew = (PRODUCTS as (Product & { isNew?: boolean })[]).filter(
      (p) => p.isNew
    );
    return flaggedNew.length ? flaggedNew : [...PRODUCTS].reverse();
  }, []);

  const filteredProducts = useMemo(() => {
    return newArrivalsList.filter((product) => {
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        {/* <div className="relative bg-[#1E3A2B] text-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl p-5 sm:p-8 mb-6 border border-[#D6A146]/30">
          ...
          <div className="relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-1.5 bg-[#D6A146]/20 border border-[#D6A146]/40 text-[#D6A146] text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded-full uppercase tracking-wider mb-2 font-btn">
              <span>Fresh Batch Arrivals</span>
            </div>

            <h1 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight mb-2 text-white">
              Latest <span className="text-[#D6A146]">New Arrivals</span>
            </h1>

            <p className="font-body text-gray-300 text-xs sm:text-sm leading-relaxed">
              Explore freshly harvested, stone-ground regional seasonings, gourmet kitchen staples, and newly crafted recipe masalas.
            </p>
          </div>
        </div> */}
        ...
        <div className="flex items-center justify-between mb-4 px-1">
          <p className="text-xs sm:text-sm text-gray-600">
            Showing <strong className="text-[#1D1D1D] font-semibold">{filteredProducts.length}</strong> new arrivals
          </p>
        </div>
